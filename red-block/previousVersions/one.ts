...
interface PlayerPhysicsObject extends PhysicsObject, InteractablePhysicsObject {
  ...
  relativeDirectionToObject: Direction;
}
...
class PlayerObject extends BaseObject implements PlayerPhysicsObject {
  ...
  isTouching(object: BlockObject) {
    ...
    Object.keys(info).forEach(key => {
      if (info[key as RelativeDirection] == true) {
        this.relativeDirectionToObject = key as RelativeDirection;
        inside = false;
      }
    });

    // console.log(inside);
    // const inside = !Object.values(info).find((value: RelativeDirections[RelativeDirection]) => value === true);

    // console.log(this.touchingSurface)
    // console.log(this.onGround)

    switch (this.relativeDirectionToObject) {
      case 'top':
        if (inside) {
          if (this.touchingSurface) this.speed.mult(V(1, 0));
  ...
  update() {
    // Checking gravity and adjusting position accordingly
    if (this.isOnGround()) {
...
const block1 = new BlockObject(V(80, HEIGHT - 60), undefined, undefined, V(80, 80));
